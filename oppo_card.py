...
def dist_plot(player,ax,team_color,team_alt_color,stat='FP',df=season_market):
    ...
    text_box = ax.text(xstat_val,ax.get_ylim()[1]*0.825,stat_dict[stat][2], ha='center', va='center', fontsize=12, weight=500,
                        color=team_color, bbox=dict(facecolor='w', alpha=1, edgecolor=team_color, linewidth=2))
    text_box.set_path_effects([patheffects.withStroke(linewidth=1,
...
def qblist_card(player, weeks, df=season_market, team_logos=pd.read_csv('https://raw.githubusercontent.com/nflverse/nflverse-pbp/master/teams_colors_logos.csv')):
    ...
    desc_ax.text(0,0.5,'Per-Game PPR Stats ({}; {})'.format(df.loc[df['player']==player,'position'].item(),week_text), 
                 ha='center', va='center', fontsize=20)
    desc_ax.text(0,-0.5,'Green Highlight: xStat for other {}'.format(df.loc[df['player']==player,'position'].item()), 
