...
def qblist_card(player, weeks, df=season_market, team_logos=pd.read_csv('https://raw.githubusercontent.com/nflverse/nflverse-pbp/master/teams_colors_logos.csv')):
    ...
    desc_ax.text(0,0.25,'Per-Game PPR Stats ({}; {})'.format(df.loc[df['player']==player,'position'].item(),week_text), 
                 ha='center', va='center', fontsize=20)
    desc_ax.text(0,-0.25,'Green Highlight: other {}'.format(df.loc[df['player']==player,'position'].item()), 
